...
def _pin_addons_in_nav(profile_dir: Path, addon_ids: list[str]) -> None:
    """Keep toolbar customization stable without injecting synthetic addon widget ids.

    Firefox/Camoufox can render pinned extension icons that do not open if widget ids
    are forced into ``browser.uiCustomization.state`` before runtime registration.
    We now only keep the unified extensions button and strip stale browser-action
    entries generated by prior launches; real pinning is handled at runtime.
    """
    ...
    merged_nav: list[str] = []
    for item in [*defaults, *nav_bar]:
        if not isinstance(item, str) or not item:
            continue
        if item.endswith("-browser-action"):
            continue
        if item not in merged_nav:
    ...
    state["seen"] = [x for x in seen if isinstance(x, str) and x and not x.endswith("-browser-action")]
