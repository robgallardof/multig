...
def _enforce_tampermonkey_instance_policies(page) -> None:
    ...
    Ensures the per-extension "Run in Private Windows" setting is "Allow".
    """
    try:
    ...
    try:
        private_state = page.evaluate(script)
        _log("INFO", "Tampermonkey private mode policy enforced", private_state=private_state)
    except Exception as exc:
...
def main() -> None:
    ...
    # Hard requirement: every launched Firefox instance must keep addons
    # enabled for private windows by default.
